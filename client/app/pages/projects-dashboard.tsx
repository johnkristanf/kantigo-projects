...
export default function ProjectsDashboardPage() {
  ...
  const tasks = [
    { id: 1, name: 'Design homepage mockup', status: 'Completed', assignee: 'Sarah Johnson', priority: 'High', weight: 'Heavy', startDate: '2026-01-10', endDate: '2026-01-15' },
    { id: 2, name: 'Implement navigation menu', status: 'In Progress', assignee: 'Mike Chen', priority: 'High', weight: 'Medium', startDate: '2026-01-16', endDate: '2026-01-22' },
    { id: 3, name: 'Create contact form', status: 'In Progress', assignee: 'Emily Davis', priority: 'Medium', weight: 'Light', startDate: '2026-01-20', endDate: '2026-01-25' },
    { id: 4, name: 'Optimize images', status: 'Pending', assignee: 'Alex Kumar', priority: 'Low', weight: 'Light', startDate: '2026-01-24', endDate: '2026-01-28' },
    { id: 5, name: 'Write documentation', status: 'Pending', assignee: 'Sarah Johnson', priority: 'Medium', weight: 'Medium', startDate: '2026-01-28', endDate: '2026-02-01' },
    { id: 6, name: 'Setup deployment pipeline', status: 'Completed', assignee: 'Mike Chen', priority: 'High', weight: 'Heavy', startDate: '2026-01-12', endDate: '2026-01-18' },
    { id: 7, name: 'Conduct user testing', status: 'Pending', assignee: 'Emily Davis', priority: 'High', weight: 'Medium', startDate: '2026-02-01', endDate: '2026-02-05' },
    { id: 8, name: 'Fix responsive issues', status: 'In Progress', assignee: 'Alex Kumar', priority: 'Medium', weight: 'Light', startDate: '2026-01-26', endDate: '2026-01-30' },
  ];
  
  const currentProject = projects.find(p => p.id === selectedProject);
  const progressPercentage = currentProject ? (currentProject.completed / currentProject.total) * 100 : 0;
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-4 mb-6">
                <Avatar className="w-16 h-16 border-4 border-white/30 shadow-xl">
                  ...
                  <AvatarFallback className="bg-white text-blue-600 text-xl font-bold">
                    {currentProject?.name.split(' ').map(word => word[0]).join('')}
                  </AvatarFallback>
                </Avatar>
                <h2 className="text-4xl font-bold tracking-tight">{currentProject?.name}</h2>
              </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-linear-to-r from-blue-50 to-indigo-50 border-b-2 border-blue-200">
                    <th className="text-left py-5 px-6 font-bold text-blue-900 text-sm uppercase tracking-wider">Task Name</th>
                    <th className="text-left py-5 px-6 font-bold text-blue-900 text-sm uppercase tracking-wider">Status</th>
                    <th className="text-left py-5 px-6 font-bold text-blue-900 text-sm uppercase tracking-wider">Assignee</th>
                    <th className="text-left py-5 px-6 font-bold text-blue-900 text-sm uppercase tracking-wider">Priority</th>
                    <th className="text-left py-5 px-6 font-bold text-blue-900 text-sm uppercase tracking-wider">Weight</th>
                    <th className="text-left py-5 px-6 font-bold text-blue-900 text-sm uppercase tracking-wider">Start Date</th>
                    <th className="text-left py-5 px-6 font-bold text-blue-900 text-sm uppercase tracking-wider">End Date</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredTasks.map((task, index) => (
                    ...
                    >
                      <td className="py-5 px-6 font-semibold text-slate-800">{task.name}</td>
                      <td className="py-5 px-6">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${getStatusColor(task.status)}`}>
                          {task.status}
                        </span>
                      </td>
                      <td className="py-5 px-6 text-slate-700 font-medium">{task.assignee}</td>
                      <td className="py-5 px-6">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${getPriorityColor(task.priority)}`}>
                          {task.priority}
                        </span>
                      </td>
                      <td className="py-5 px-6">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${getWeightColor(task.weight)}`}>
                          {task.weight}
                        </span>
                      </td>
                      <td className="py-5 px-6">
                        <div className="flex items-center gap-2 text-slate-700">
                          ...
                          <span className="font-medium text-sm">{task.startDate}</span>
                        </div>
                      </td>
                      <td className="py-5 px-6">
                        <div className="flex items-center gap-2 text-slate-700">
                          <Calendar className="w-4 h-4 text-indigo-500" />
                          <span className="font-medium text-sm">{task.endDate}</span>
                        </div>
                      </td>
                    </tr>
                  ))}
